...
export async function chatJson<T>(
  ...
  try {
    ...
    if (!response.ok) {
      throw new ModelApiError(await parseError(response), response.status);
    }
    ...
    if (!content) throw new ModelApiError('模型没有返回内容');

    return extractJsonObject(content) as T;
  } catch (err) {
    if (err instanceof ModelApiError) throw err;
    const e = err as Error;
    if (e.name === 'AbortError') throw new ModelApiError('模型请求超时');
