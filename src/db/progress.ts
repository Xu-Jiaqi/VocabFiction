...
export async function getProgress(workId: string): Promise<ReadingProgress | null> {
  try {
    ...
    console.warn('[Progress] getProgress error:', (e as Error)?.message ?? String(e));
    return null;
  }
}
...
export async function updateProgress(
  ...
  try {
    ...
    const current: ReadingProgress = existing ?? {
      work_id: workId,
      current_ep: 1, current_msg: 0, total_read_eps: 0,
      status: 'reading', started_at: null, last_read_at: null,
    };
    ...
    if (existing) {
      await db.runAsync(
        'UPDATE reading_progress SET current_ep = ?, current_msg = ?, total_read_eps = ?, status = ?, last_read_at = datetime(\'now\') WHERE work_id = ?',
        [merged.current_ep, merged.current_msg, merged.total_read_eps, merged.status, workId]
      );
    } else {
      await db.runAsync(
        'INSERT INTO reading_progress (work_id, current_ep, current_msg, total_read_eps, status, started_at, last_read_at) VALUES (?, ?, ?, ?, ?, datetime(\'now\'), datetime(\'now\'))',
        [workId, merged.current_ep, merged.current_msg, merged.total_read_eps, merged.status]
      );
    }
  } catch (e) {
    console.warn('[Progress] updateProgress error:', (e as Error)?.message ?? String(e));
  }
}
